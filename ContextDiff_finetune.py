...
class MultiCLIP(torch.nn.Module):
    def __init__(self, device="cuda",dtype=torch.float32):
        super().__init__()
        model_32, _ = clip.load("ckpt/ViT-B-32.pt", device=device)
        model_16, _ = clip.load("ckpt/ViT-B-16.pt", device=device)
        model_101, _ = clip.load("ckpt/RN101.pt", device=device)
        self.dtype=dtype
    ...
    def encode_image(self, image, dtype=torch.float16):
        with torch.no_grad():
            ...
            #return tensor, with shape of (batch_size, 512*3)
...
def train(
    ...
    # for key in test_pipeline_config:
    #     print(key, test_pipeline_config[key])
    ...
    use_local_shift=True
    dtype=unet.parameters().__next__().dtype

    params_to_optimize = unet.parameters()
    if use_local_shift:
        clip_model=MultiCLIP(dtype=dtype)
        clip_model.to(accelerator.device)
        clip_localshift=CLIPLocalShift(uselocal=use_local_shift).to(accelerator.device).to(dtype)
    else:
        clip_model=None
        clip_localshift=None
    ...
    if use_local_shift:
        params_to_optimize = list(unet.parameters())+list(clip_localshift.parameters())
    else:
        params_to_optimize = unet.parameters()
    ...
    # for key in trainer_pipeline_config:
    #     print(key, trainer_pipeline_config[key])
    ...
    # print('editing_config')
    # print(editing_config.editing_prompts)
    ...
    while step < train_steps:
        ...
        if use_local_shift:
            loss = trainer.step(batch,train_dataset_config["prompt"])
        else:
        ...
        if accelerator.sync_gradients:
            ...
            if accelerator.is_main_process:
                ...
                if validation_sample_logger is not None and (step % validation_steps == 0):
                    ...
                    if use_local_shift:
                        image_emb=clip_model.encode_image(val_image)
                        text_emb=clip_model.encode_text(train_dataset_config.prompt, accelerator.device)
                        local_shift=clip_localshift(image_emb, text_emb)
                        local_shift=rearrange(local_shift, "( b f ) ( c h w ) -> b c f h w", b=1,f=8,h=8,w=8)
                        print(str(local_shift.shape))
                    ...
                    if editing_config.get('use_invertion_latents', False):
                        ...
                        batch['latents_all_step'] = pipeline.prepare_latents_ddim_inverted(
                            ...
                            context_shift=local_shift if use_local_shift else None,
                            )
