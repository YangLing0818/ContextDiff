...
class UNetPseudo3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        context_awareness_shift:Optional[torch.Tensor] = None
        ...
        # torch.Size([1, 1280, 2, 8, 8])
        # torch.Size([1, 1280, 2, 8, 8])
        if context_awareness_shift is not None:
            print('use context_awareness_shift')
            print('sample'+str(sample.shape))
            sample = sample + context_awareness_shift
        # 5. up
    ...
    @classmethod
    def from_2d_model(cls, model_path, model_config,use_tuneavideo=False):
