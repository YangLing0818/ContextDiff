...
class ClipPrior(nn.Module):
    def __init__(self, xf_width=1024, xf_layers=16, xf_heads=32, clip_width=512*3, learn_sigma=False, t5_dim=1024,
                ...
                vocab_log_std_init=0., vocab_mean_init=0., vocab_learnable=False, vocab_std_scale=4., vocab_lr_scale=1., vocab_exp=False,local_linear=False,
                local_Bilinear=False,cross_attention=False):
        super().__init__()
        self.local_linear = local_linear
        self.local_Bilinear = local_Bilinear
        self.xf_width = xf_width 
        self.local_linear_feature=None
        self.local_Bilinear_feature=None
        ...
        #concate the text and image feature
        if local_linear:
            self.image_text_linear= nn.Sequential(
                nn.Linear(clip_width*2, clip_width)
            )
            
        if local_Bilinear:
            self.bilinear_layer = CompactBilinearPooling(clip_width, clip_width, clip_width)  
    ...
    def forward(self, x, timesteps, t5_word_emb, clip_sentence_emb, emb_4_vocab, indices=None, testing=False,**model_kwargs):
        ...
        if self.training:
            if self.local_linear:
                self.local_linear_feature=self.image_text_linear(torch.cat([clip_sentence_emb,model_kwargs['clip_image']],dim=1))
            if self.local_Bilinear:
                self.local_Bilinear_feature=self.bilinear_layer(clip_sentence_emb.to(torch.float32), model_kwargs['clip_image'].to(torch.float32)).to(torch.float16)
