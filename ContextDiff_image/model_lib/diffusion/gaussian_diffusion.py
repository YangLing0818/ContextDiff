...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None,linear=None,Bilinear=None#, emb_4_vocab=None
    ): #DONE
        """
        ...
        if self.vocab is not None:
            ...
            print("_mean:"+str(_mean.shape))
            print("_std:"+str(_std.shape))
            print("_indices:"+str(_indices.shape))
            
            print("p_sample:linear:"+str(linear))
            print("p_sample:bilinear:"+str(Bilinear))
            if linear is not None:
                print("use linear")
                _mean = _mean + linear
            if Bilinear is not None:
                print("use Bilinear")
                _mean = _mean + Bilinear
            sft_var = (_std.to(device=t.device)*self.vocab.std_scale)**2
        else:
    ...
    def p_sample(
        ...
        if linear is not None:
            noise = linear*(8-t[0])*0.125
        if Bilinear is not None:
            noise = Bilinear*(8-t[0])*0.125
        if linear is None and Bilinear is None:
    ...
    def p_sample_loop_progressive(
        ...
        if self.vocab is not None:
            ...
            print("emb_4_vocab"+str(model_kwargs['emb_4_vocab'].shape))
            _std = _std * self.vocab.std_scale
            img = img * _std.to(device=device)
            img = img + _mean.to(device=device)

        indices = list(range(self.num_timesteps))[::-1]
        print("linear:"+str(use_linear))
        print("bilinear:"+str(use_bilinear))
        print("+model_kwargs['emb_4_vocab'][: len(model_kwargs['emb_4_vocab']) // 2]:"+str(model_kwargs['emb_4_vocab'][: len(model_kwargs['emb_4_vocab']) // 2].shape))
        if progress:
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                print("pred_xstart:"+str(out['pred_xstart'].shape))
                if clip_model is not None:
                    if prior_model is not None:
                        if use_linear:
                            linear=prior_model.image_text_linear(th.cat((model_kwargs['emb_4_vocab'],out["pred_xstart"]),dim=1))
                        if use_bilinear:
                            bilinear=prior_model.bilinear_layer(out["pred_xstart"],model_kwargs['emb_4_vocab'][: len(model_kwargs['emb_4_vocab']) // 2])
                img = out["sample"]
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, use_d=False, discriminator=None):
        ...
        if noise is None:
            noise = th.randn_like(x_start)
        print("Initial model_kwargs:")
        # for key in model_kwargs:
        #     print(key)
        if self.vocab is not None:
            _indices, _mean, _std = self.vocab.get_indices(model_kwargs['emb_4_vocab'],) 
            if model_kwargs['use_linear']:
                _mean=model.model.image_text_linear(th.cat([model_kwargs['clip_sentence_emb'],model_kwargs['clip_image']],dim=1))+_mean
            elif model_kwargs['use_bilinear']:
                _mean=model.model.bilinear_layer(model_kwargs['clip_sentence_emb'].to(th.float32), x_start.to(th.float32)).to(th.float16)+_mean
